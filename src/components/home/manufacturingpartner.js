...
const ManufacturingPartner = ({ manufacturingPartnerData }) => {
    return (
        <section className="bg-dark-sky txt-white pt-80 manufacture-partner-sec manu-parter-reverse">
            <div className="container">
                <div className="pb-46 manu-parter-title">
                    <h2>{manufacturingPartnerData?.sectionTitle}</h2>
                </div>
            </div>
            <div className="manu-parter-dls">
                <div className="manu-parter-dls-img">
                    <img src={manufacturingPartnerData?.image?.node?.sourceUrl} alt={manufacturingPartnerData?.image?.node?.altText || manufacturingPartnerData?.image?.node?.title} />
                </div>
                <div className="container">
                    <div className="manu-parter-dls-txt">
                        <h4>{manufacturingPartnerData?.subHeading}</h4>
                        <div dangerouslySetInnerHTML={{ __html: manufacturingPartnerData?.description }} />
                        {manufacturingPartnerData?.ctaButton ? (
                            <a href={manufacturingPartnerData?.ctaButton?.url} className="btn btn-primary" target={manufacturingPartnerData?.ctaButton?.target}>
                                {manufacturingPartnerData?.ctaButton?.title}
                                <span>
